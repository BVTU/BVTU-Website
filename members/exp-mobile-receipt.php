<?php
/**
 * exp-mobile-receipt.php — Phone receipt uploader (token-gated, no login required)
 * URL: /members/exp-mobile-receipt.php?token=XXXX
 */
...
$tokenRow = $token ? expValidateUploadToken($token) : null;
$expense  = $tokenRow ? expGet((int)$tokenRow['expense_id']) : null;
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="card">
  ...
  <?php if (!$expense): ?>
    ...
    <div class="error-card" style="display:block;">
      ...
      <h2>Invalid Link</h2>
      <p>This receipt upload link is not valid. Make sure you scanned the QR code from the expense submission page.</p>
    </div>

  <?php else: ?>
    <!-- ── Upload UI ── -->
    <div class="expense-label">Uploading receipt for</div>
    <div class="expense-name"><?= htmlspecialchars(
        strlen($expense['description']) > 60
            ? substr($expense['description'], 0, 57) . '...'
            : $expense['description']
    ) ?></div>
    ...
    <div class="loading" id="loadingState">
      ...
      <div class="loading-text" id="loadingText">Uploading&hellip;</div>
    </div>
    ...
    <div class="success" id="successState">
      ...
      <p style="font-size:.85rem;color:#6b7280;">It's waiting in your desktop expense form &mdash; look for the green tray.</p>
      <div class="success-details" id="scanDetails" style="display:none;">
        ...
        <div class="row"><span class="label">Category</span><span class="value" id="detailCat">&#x2014;</span></div>
      </div>
...
function show(id) {
    ...
    for (var i = 0; i < states.length; i++) {
        ...
        if (el) {
            el.style.display = (states[i] === id)
                ? (states[i] === 'loadingState' ? 'flex' : 'block')
                : 'none';
        }
    }
}
...
function handleFile(input) {
    ...
    fetch('exp-mobile-scan.php', { method: 'POST', body: fd })
        ...
        .then(function(d) {
            ...
            if (d.vendor || d.amount || d.date || d.category) {
